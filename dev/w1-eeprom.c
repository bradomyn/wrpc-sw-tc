...
/*
 * The OneWire EEPROM contains important information for White Rabbit protocol: Transition Phase
 * and SFP calibration parameters, networking: mac addresse  and an init script area consist of
 * 2-byte size field and a set of  shell  commands.
 *
 ...
 *       -------------------------------------------------------------------------------
 *0x240  |   SFP(1) part number (16B)        | alpha (4B) | deltaTx (4B) | deltaRx (4B) |
 *       --------------------------------------------------------------------------------
...
int w1_eemprom_sfpdb_erase()
{
	...
	int res;
	res =
	    w1_read_eeprom_bus(&wrpc_w1_bus, EE_BASE_SFP, (uint8_t *) buf,
			       W1_BUF);
	buf[4] &= 0x0;		/*| cal_ph_trans (4B) | SFP count (1B) | */
	res +=
	    w1_write_eeprom_bus(&wrpc_w1_bus, EE_BASE_SFP, (uint8_t *) buf,
				W1_BUF);
...
int w1_eeprom_get_sfp(struct s_sfpinfo *sfp, uint8_t add, uint8_t pos)
{
	...
	if (!add) {
		...
		if (w1_read_eeprom_bus
		    (&wrpc_w1_bus, ((pos + 1) * EE_BASE_CAL + EE_BASE_SFP),
		     (uint8_t *) ptr, W1_BUF) < 0)
			return EE_RET_1WERR;
...
int w1_eeprom_phtrans(uint32_t * val, uint8_t write)
{
	...
	if (write) {
		...
		*val |= (1 << 31);
		if (w1_read_eeprom_bus(&wrpc_w1_bus, EE_BASE_SFP, buf, W1_BUF) <
		    0)
			return EE_RET_1WERR;

		memcpy(buf, val, 4);

		if (w1_write_eeprom_bus(&wrpc_w1_bus, EE_BASE_SFP, buf, W1_BUF)
		    < 0)
			return EE_RET_1WERR;
		else
			return 0;
...
int w1_eeprom_init_add(const char *args[])
{
	...
	if (w1_write_eeprom_bus(&wrpc_w1_bus, EE_BASE_INIT, num_cmd, W1_BUF) <
	    0)
		return EE_RET_1WERR;
	return 0;
}
...
int w1_eeprom_init_show()
{
	...
	for (i = 1; i <= used; i++) {
		...
		if (w1_read_eeprom_bus
		    (&wrpc_w1_bus, EE_BASE_INIT + (EE_BASE_CAL * i), buf,
		     W1_BUF) < 0)
			return EE_RET_1WERR;
...
static int cmd_w1_w(const char *args[])
{
	...
	if (args[0] == '\0' || args[1] == '\0') {
		pp_printf
		    ("This commands needs arguments, w1w PAGE_EEPROM STRING\n");
		return 0;
	}
...
static int cmd_w1_r(const char *args[])
{
	...
	page = atoi(args[0]);
	w1_read_eeprom_bus(&wrpc_w1_bus, page * 0x20, (uint8_t *) pn, 32);
...
static int cmd_w1_test(const char *args[])
{
	...
	for (page = 0; page < 80; page++) {
		w1_read_eeprom_bus(&wrpc_w1_bus, page * 0x20, (uint8_t *) pn,
				   32);
