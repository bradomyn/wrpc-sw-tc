...
static int cmd_sfp(const char *args[])
{
	...
	if (args[0] && !strcasecmp(args[0], "detect")) {
		...
		temp = w1_eeprom_get_sfp(&sfp, 1, 0);

		if (temp == EE_RET_DBFULL)
			pp_printf("SFP DB is full\n");
		else if (temp == EE_RET_1WERR)
			pp_printf("oneWire error\n");
		else
		...
		for (i = 0; i < sfpcount; ++i) {
			...
			memset(&sfp, 0, 32);
			temp = w1_eeprom_get_sfp(&sfp, 0, i);
			...
			if (i == 0) {
				...
				if (sfpcount == 0 || sfpcount == 0xFF) {
					...
					pp_printf("%d SFPs in data base\n",
						  sfpcount);
			}

			if (temp < 0) {
				pp_printf("Error reading the EEPROM \n");
			} else {
				pp_printf("%d: PN:", i + 1);
				for (j = 0; j < 16; ++j)
					mprintf("%c", sfp.pn[j]);
		...
		if (w1_eeprom_match_sfp(&sfp) > 0) {
			...
			pp_printf("SFP not present in the db\n");
		return 0;
	}
